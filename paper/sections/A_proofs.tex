...
\paragraph{Proposition~\ref{thm:stoch} (Sample-average stochastic consistency).}
If $x$ strictly dominates $y$ under all scenarios, then $r(x, \omega^{(s)}) \le r(y, \omega^{(s)})$ for all $s$, with strict inequality on at least one coordinate. For any separating monotone probe $q$, $q(r(x, \omega^{(s)})) \le q(r(y, \omega^{(s)}))$ for all scenarios, and strictly less for some separating probe. The finite sample average $\hat\mu_q(x) = \frac{1}{S}\sum_{s} q(r(x, \omega^{(s)}))$ strictly preserves this inequality. Since the normalisation constants $q^\star$ and $q^{\mathrm w}$ are fixed across alternatives, the corresponding regret is strictly lower for $x$, giving $x \prec_{\mathrm{LUR}} y$ by the exact same logic as Theorem~\ref{thm:pareto}. \hfill$\square$
