...
attains both endpoints on $A$. If $\nadir_i - \ideal_i$ approaches zero, the normalisation floor $\varepsilon$ prevents division by zero and effectively zeros out the relevance of that criterion. In practice, analysts should check if $\nadir_i - \ideal_i < 10^{-6}$; if so, the criterion is degenerate on the candidate set and should be excluded from probe generation to avoid numerical noise.
