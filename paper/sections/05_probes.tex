...
recommendation. We need a probe family that (i) is small, (ii) stays monotone,
and (iii) is less sensitive to redundancy than averaging aggregation.

\subsection{Correlation clustering}
We group \emph{redundant} criteria by single-linkage connected components of the
graph whose edges join pairs with Pearson correlation $\Corr(f_i,f_j)\ge\theta$.
